...
def transformer(U, theta, out_size, name='SpatialTransformer', **kwargs):
    ...
    def _transform(theta, input_dim, out_size):
        with tf.variable_scope('_transform'):
            ...
            num_batch = shape[0]
            height = tf.cast(shape[1], 'float32')
            width = tf.cast(shape[2], 'float32')
            num_channels = shape[3]
            theta = tf.reshape(tf.cast(theta, 'float32'), (-1, 3))
            ...
            # Transform A x (x_t, y_t, 1)^T -> (x_s, y_s) of shape [batch*2, -1]
            T_g = tf.reshape(tf.matmul(A, grid), [num_batch, 2, -1])
